...
Aggregate는
하나 이상의 Contract를 조직하는
논리적인 상위 단위(Logical Organization Unit)이다.

Aggregate는 Runtime 실행 단위가 아니다.

Aggregate는 Organization,
Dependency,
Visibility,
Packaging을 담당한다.

Module Contract는
현재 Specification에서 Aggregate를 표현하는
대표적인 Contract이다.
...
Package는
Contract를 논리적으로 조직하는 단위이다.

현재 Specification에서는
Module Contract가
Package Organization을 표현한다.

향후 Package Contract가
정의될 수 있다.
...
Atomic Layer는 언어를 구성하는 가장 작은 표현 단위를 정의하는 Architecture Layer이다.

Atomic Layer는 다음 Contract로 구성된다.
...
Callable Layer는 호출 가능한 Contract를 조직하는 Architecture Layer이다.

Callable Layer는 다음 Contract로 구성된다.
...
Object Layer는 Atomic Layer와 Callable Layer를 조합하여 객체 구조를 표현하는 Architecture Layer이다.

현재 Specification에서는 Class Contract가 Object Layer를 담당한다.

---

# Aggregation Layer

Aggregation Layer는 Object Layer를 조직하는 최상위 Architecture Layer이다.

현재 Specification에서는 Module Contract가 Aggregation Layer를 담당한다.
...
Registry는 Identity를 기준으로 Contract를 등록, 조회 및 관리한다.

Registry는 Contract의 유일한 공식 저장소(Canonical Repository)이다.

Registry View는 Registry에 저장된 Contract의 논리적인 표현(Logical View)이며,
독립적인 Registry 인스턴스를 의미하지 않는다.

Module Registry와 같은 용어는 Registry View의 한 종류를 의미하며,
별도의 Registry를 생성하는 것을 의미하지 않는다.
