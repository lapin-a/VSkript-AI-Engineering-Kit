...
Registry는
모든 Contract의 공식 Source of Truth이다.
...
프로젝트는 문서 계층(Document Hierarchy)과
Contract Architecture Layer를 함께 따른다.
...
하위 계층은
상위 계층에 직접 의존해서는 안 된다.

동일 계층 간 의존은
Specification에서 명시적으로 허용된 경우에만 가능하다.

Generator는 Contract Architecture Layer를 변경해서는 안 된다.

Canonical Structure는 각 Layer의 공식 표현이다.
...
하나의 Domain Object는 하나의 Canonical Model만 가진다.
...
Canonical Structure는
Contract의 공식 구조(Source of Truth)이다.

Generator는 Canonical Structure를 기준으로
Artifact를 생성해야 한다.
...
Contract는 Architecture Layer를 따른다.

상위 Layer는
하위 Layer를 조직할 수 있다.

하위 Layer는
상위 Layer를 직접 참조해서는 안 된다.

동일 Layer 간 의존은
Specification에서 명시적으로 정의된 경우에만 허용된다.
...
Core Contract는 프로젝트 전체에서 공통적으로 적용되는 기본 정책(Default Policy)만 정의한다.

플랫폼별 구현, 언어별 차이, 선택적 기능 및 실험적인 기능은 Core Contract에 포함하지 않는다.

이러한 기능은 별도의 Specification을 통해 확장한다.

---

## Objectives

Default Policy Principle의 목적은 다음과 같다.

- Core Contract를 단순하게 유지한다.
- Contract의 장기적인 안정성을 보장한다.
- Generator의 예측 가능성을 높인다.
- 플랫폼별 구현을 분리한다.
- 확장을 독립적인 Specification으로 관리한다.

---

## Rules

Core Contract는 다음만 정의한다.
...
다음 내용은 Core Contract에서 정의하지 않는다.
...
추가 기능은 별도의 Specification에서 정의한다.
...
Core Contract는 Extension을 직접 포함하지 않는다.

---

## Generator Responsibilities

Generator는 항상 Core Contract의 Default Policy를 구현해야 한다.

Extension Specification은 Generator가 선택적으로 지원할 수 있다.

Generator는 지원하지 않는 Extension을 무시하거나 Diagnostic을 생성해야 한다.

---

## Compatibility

Default Policy는 가능한 한 변경하지 않는다.

새로운 기능은 기존 정책을 수정하지 않고 Extension Specification으로 추가한다.

Core Contract의 의미를 변경하는 것은 Breaking Change로 간주한다.

---

## Summary

Core Contract는 프로젝트의 안정적인 기반(Base Layer)이다.

모든 확장은 Core를 수정하지 않고 별도의 Specification을 통해 정의한다.

이를 통해 장기적인 유지보수성과 확장성을 확보한다.